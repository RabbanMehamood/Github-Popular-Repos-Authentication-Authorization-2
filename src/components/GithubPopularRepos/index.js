...
class GithubPopularRepos extends Component {
  ...
  getRepositories = async () => {
    ...
    if (response.ok) {
      ...
      const updatedData = fetchedData.popular_repos.map(eachRespository => ({
        id: eachRespository.id,
        imageUrl: eachRespository.avatar_url,
        name: eachRespository.name,
        starsCount: eachRespository.stars_count,
        forksCount: eachRespository.forks_count,
        issuesCount: eachRespository.issues_count,
      }))
      this.setState({
  ...
  renderRepositoriesListView = () => {
    ...
    return (
      <ul className="repositories-list">
        {repositoriesData.map(eachRespository => (
          <RepositoryItem
            key={eachRespository.id}
            repositoryDetails={eachRespository}
          />
        ))}
      </ul>
    )
